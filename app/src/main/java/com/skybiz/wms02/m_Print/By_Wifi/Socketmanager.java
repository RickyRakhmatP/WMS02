...
public class Socketmanager {
    ...
    public void threadconnectwrite(byte[] str, String FontSize)
    {
        new WriteThread(str, FontSize);
    }
    ...
    public boolean ConnectAndWrite(byte[] out, String FontSize)
    {
        if(connect()) {
            //byte[] fs={27,72,42};
            byte[] blarge = new byte[]{0x1B, 0x21, 0x10};
            byte[] bnormal = new byte[]{0x1B, 0x21, 0x00};
            if (FontSize.equals("Enlarged")){
                write(blarge);
            }else{
                write(bnormal);
            }
    ...
    private class WriteThread extends Thread
    {
        byte[] out;
        String FontSize;
        public WriteThread(byte[] str, String iFontSize)
        {
            out=str;
            FontSize=iFontSize;
            start();
        ...
        public void run()
        {
            if(ConnectAndWrite(out,FontSize))
            {
                SetState(MESSAGE_WRITE_SUCCESS);
            }
        }
