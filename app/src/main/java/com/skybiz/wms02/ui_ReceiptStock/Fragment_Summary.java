...
public class Fragment_Summary extends Fragment {
        ...
        btnPay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(PaidAmount.equals("0.00") || PaidAmount.length()==0){
                    ...
                    SaveTrn fnsave=new SaveTrn(getActivity(),"RS",Doc1No,"");
                    fnsave.execute();
                    btnPay.setEnabled(true);
                    txtDoc1No.setText("");
                }
    ...
    private void fnchecktotal(){
        ...
        DBAdapter db = new DBAdapter(getContext());
        db.openDB();
        String vQuery="select sum(HCLineAmt + HCDiscount) as TotalAmt, sum(QtyOUT) as TotalDiscount " +
                " from cloud_inventory_dt where DocType='RS' ";
    ...
    private void retLastNo(){
        ...
        DBAdapter db = new DBAdapter(getContext());
        db.openDB();
        String vQuery="select Prefix,LastNo from sys_runno_dt where RunNoCode='RS' ";
        Cursor cRunNo = db.getQuery(vQuery);
        while (cRunNo.moveToNext()) {
    ...
    public void refreshPayment() {
        retLastNo();
        CheckPaidAmt chekpaid=new CheckPaidAmt(getActivity(),"RS");
        fnchecktotal();
