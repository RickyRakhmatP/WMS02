...
public class DonwloadBay extends AsyncTask<Void,Void,String> {
    Context c;
    ...
    public DonwloadBay(Context c, String Keyword, RecyclerView rv, DialogBay dialogBay) {
        this.c = c;
    ...
    protected void onPostExecute(String result) {
        ...
        if(result==null){
            ...
            BayParser p=new BayParser(c,result,rv,dialogBay);
            p.execute();
        }
    }
    ...
    private String downloadData(){
        try {
            ...
            DBStatus="0";
            String sql ="Select Bay from wms_loading_bay where Bay like '%"+Keyword+"%' ";
            if(DBStatus.equals("1")){
                ...
                while(rsData.moveToNext()){
                    JSONObject row=new JSONObject();
                    row.put("Bay",rsData.getString(0));
                    results.put(row);
                }
                db.closeDB();
