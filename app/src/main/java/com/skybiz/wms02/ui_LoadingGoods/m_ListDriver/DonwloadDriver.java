...
public class DonwloadDriver extends AsyncTask<Void,Void,String> {
    Context c;
    ...
    public DonwloadDriver(Context c, String SearchBy, String Keyword,
                          RecyclerView rv, DialogDriver dialogChecker) {
        this.c = c;
    ...
    protected void onPostExecute(String result) {
        ...
        if(result==null){
            ...
            DriverParser p=new DriverParser(c,result,rv,dialogChecker);
            p.execute();
        }
    }
    ...
    private String downloadData(){
        try {
            ...
            if(SearchBy.equals("By Code")){
                vClause=" where DriverCode like '%"+Keyword+"%' ";
            }else if(SearchBy.equals("By Name")){
                vClause=" where DriverName like '%"+Keyword+"%' ";
            }

            String sql ="Select DriverCode, DriverName from wms_driver "+vClause+" ";
            if(DBStatus.equals("1")){
                ...
                while(rsData.moveToNext()){
                    JSONObject row=new JSONObject();
                    row.put("DriverCode",rsData.getString(0));
                    row.put("DriverName",rsData.getString(1));
                    results.put(row);
                }
                db.closeDB();
