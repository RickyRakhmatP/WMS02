...
public class DownloadCustomer extends AsyncTask<Void,Void,String> {
    Context c;
    String SearchBy,Keyword,FinCatCode;
    RecyclerView rv;
    String IPAddress,UserName,Password,DBName,Port,URL,z,DBStatus,ItemConn;
    DialogCustomer dialogCustomer;

    public DownloadCustomer(Context c, String FinCatCode, String SearchBy,
                            String Keyword, RecyclerView rv, DialogCustomer dialogCustomer) {
        this.c = c;
        this.FinCatCode = FinCatCode;
    ...
    private String downloadData(){
        try {
            ...
            if(FinCatCode.equals("All")){

            }else{
                vClause +=" and FinCatCode='"+FinCatCode+"' " ;
            }
            if(DBStatus.equals("1")){
                ...
                if (conn != null) {
                    String sql ="Select CusCode,CusName,TermCode,D_ay,IFNULL(SalesPersonCode,'') as SalesPersonCode " +
                            "from customer where StatusBadYN='0' "+vClause+"  ";
                    Log.d("QUERY",sql);
