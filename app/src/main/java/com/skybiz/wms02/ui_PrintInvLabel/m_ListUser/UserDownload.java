...
public class UserDownload extends AsyncTask<Void,Void,String> {

    Context c;
    RecyclerView rv;
    String IPAddress,UserName,Password,DBName,Port,URL,z,DBStatus,ItemConn;

    public UserDownload(Context c, RecyclerView rv) {
        this.c = c;
        this.rv = rv;
    }
    ...
    protected void onPostExecute(String result) {
        ...
        if(result==null){
            Toast.makeText(c,"error get list user", Toast.LENGTH_SHORT).show();
        }else{
            UserParser p=new UserParser(c,result,rv);
            p.execute();
        }
    }
    ...
    private String downloadData(){
        try {
            ...
            curSet.close();
            DBStatus="0";
            String sql ="Select UserCode, UserName from sys_userprofile_hd " +
                    "where Status='Active' and DepartmentCode='BILLING' ";
            if(DBStatus.equals("1")){
                ...
                while(rsData.moveToNext()){
                    ...
                    row2.put("UserCode",rsData.getString(0));
                    row2.put("UserName",rsData.getString(1));
                    results2.put(row2);
                }
                db.closeDB();
